...
<div class="operation operation-${this._esc(op.method)}">
  <h3><span class="method">${op.method.toUpperCase()}</span> ${op.path}${op.operationId ? ' – ' : ''}${this._esc(op.operationId || '')}</h4>
  ...
  visitResponse(response) {
    return `
<div class="response">
  <h5><strong>${this._esc(response.statusCode)}</strong> – ${mdInline(response.description || '')}</h5>
  ...
  visitSchema(schema) {
    ...
    const schemaTable = `<h6>Schema</h6>
