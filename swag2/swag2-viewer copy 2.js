...
<div class="swagger-doc">
  ...
  visitPathItem(pathItem) {
    ...
    return `
<div class="path-item">
  <h2 class="path">${this._esc(pathItem.path)}</h2>
  ${opsHtml}
</div>`.trim();
  }
...
<div class="operation operation-${this._esc(op.method)}">
  <h3><span class="method">${op.method.toUpperCase()}</span> ${op.path}${op.operationId ? ' – ' : ''}${this._esc(op.operationId || '')}</h3>
...
<table class="example-table">
  ...
  <thead><tr><th>Atributo / Valor</th></tr></thead>
  <tbody>${exampleRows
    .map(
      (r) => `<tr>
          <td style="padding-left: ${r.depth * 20}px"><span class="key">${this._escKey(r.name)}</span>
					<span class="tag">${r.tag || ''}</span><span class="value-cell">${r.value || ''}</span>${(r.comma && '<span class="end">,</span>') || ''}
					</td>
        </tr>`
    )
    .join('\n')}</tbody>
  ...
  _collectExampleRows(properties, required, depth, visited, rows) {
    ...
    for (const [name, propSchema] of properties) {
      ...
      if (expanded.kind === 'primitive') {
        rows.push({ depth, name, value: this._formatExampleValue(this._generateExampleValue(expanded.schema)), comma });
        continue;
      }

      if (expanded.kind === 'cycle') {
        rows.push({ depth, name, value: '<em>(circular)</em>', comma });
        continue;
      }

      if (expanded.kind === 'object') {
        rows.push({ depth, name, tag: '{' });
        const nextVisited = expanded.refName ? new Set(visited).add(expanded.refName) : visited;
      ...
      if (itemExpanded.kind === 'primitive') {
        ...
        rows.push({ depth, name, tag: '[', value: `${this._formatExampleValue(val)}`, end: ']', comma });
