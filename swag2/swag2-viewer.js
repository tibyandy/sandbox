...
class HtmlRenderer extends SwaggerVisitor {
  ...
  visitDocument(doc) {
    ...
    // const pathsHtml = [...doc.paths.values()].map((p) => p.accept(this)).join('\n');
		// console.log(Object.values(doc.tagsMap))
		const pathsHtml = doc.tagsRegistry.values().map((to) => {
  ...
  visitTag(tag) {
    const opsHtml = [...tag.operations.values()].map((op) => op.accept(this)).join('\n');
    return `
<div class="tag-item">
  ${opsHtml}
</div>`.trim();
  ...
  visitOperation(op) {
		const parametersHtml = op.parameters.map((p) => p.accept(this)).filter(x => !!x).join('\n')
    const paramsHtml = parametersHtml
      ? `<div class="parameters"><h3>Parâmetros</h3><table>
          <thead><tr><th>Tipo</th><th>Nome</th><th>Descrição</th></tr></thead>
          <tbody>${parametersHtml}</tbody>
        </table></div>`
      : '';

    // Corpo (body) tem seu próprio schema — renderiza como bloco à parte, igual response.schema,
    // em vez de tentar encaixar a tabela de schema dentro da célula da tabela de parâmetros.
    const bodyParam = op.parameters.find((p) => p instanceof BodyParameter);
    const bodySchemaHtml = bodyParam && bodyParam.schema
      ? `<div class="body-schema"><h3>Corpo da requisição</h3>${bodyParam.schema.accept(this)}</div>`
      : '';
...
<div class="operation operation-${this._esc(op.method)}">
		<table>
			<thead><tr><th class="header"><h2><span class="tag">${op.tags[0]} &ndash;</span> <span class="method">${op.method.toUpperCase()}</span></h2> ${op.path}${op.summary ? ` – <span class="summary">${this._esc(op.summary)}</span>` : ''}</th></tr>
		  	<tr><td>${op.description ? `<span class="description">${mdToHtml(op.description)}</span>` : ''}</td></tr>
			</thead>
		<tbody><tr><td>
  	${/*this._esc(op.operationId || '')}${op.operationId ? ' – ' : */''}
  ${paramsHtml}
  ${bodySchemaHtml}
  <div class="responses">${responsesHtml}</div>
</td></tr></tbody></table></div>`.trim();
  }
  ...
  visitParameter(param) {
    ...
    return `<tr>
			...
			<td>${this._esc(param.name)}${param.required ? '<span class="required">*</span>' : ''}</td>
      <td class="description">${mdToHtml(param.description) || this._esc(type)}</td>
    </tr>`;
  }
  ...
  visitResponse(response) {
    return `
<div class="response">
  <h3>Resposta ${this._esc(response.statusCode)}${response.description && (' – ' + mdInline(response.description) || '')}</h3>
  ...
  visitSchema(schema) {
    ...
    const rowsHtml = rows
      .map(
        (r) => `<tr>
          <td style="padding-left: ${(r.depth + .5) * 20}px"><span class="attribute">${this._esc(r.name)}</code>${r.required ? '<span class="required">*</span>' : ''}<span class="colon">:</span>
          <span class="type-cell ${r.cssType}">${r.type}</span></td>
          <td style="padding-left: ${(r.depth + .5) * 15}px">${r.description}</td>
        </tr>`
      )
      .join('\n');

    const schemaTable = `<table class="schema-table">
  <thead><tr><th>Atributo / Tipo</th><th>Descrição</th></tr></thead>
  <tbody>${rowsHtml}</tbody>
    ...
    const exampleTable = exampleRows.length
      ? `<h4>Exemplo</h4>
			<pre class="example"><div><span class="tag">{</span></div>${
				exampleRows.map((r) => `<div class="${r.type}">${'<i>  </i>'.repeat(r.depth + 1)}${this._escKey(r.name)}<span class="tag">${r.tag || ''}</span><span class="value">${r.value || ''}</span>${(r.comma && '<span class="sep">,</span>') || ''}</div>`).join('')}<div><span class="tag">}</span></div></pre>`.trim()
      : '';

    return `<div class="two-columns"><div>${exampleTable}</div><div>${schemaTable}</div></div>`;
  }
  ...
  _expand(schema, visited) {
    ...
    if (schema.allOf && schema.allOf.length) {
      ...
      let description = schema.description || null;
      let inheritedDescription = null; // vem de membro $ref — só usada se nada mais específico existir
      ...
      for (const member of schema.allOf) {
        ...
        // Membro pode ser só `{ "description": "..." }` sem type/properties — isObject() não
        // reconhece isso como objeto (vira 'primitive'), mas a description ainda é válida e
        // tem PRIORIDADE sobre a herdada de um $ref (é a description específica desta composição).
        if (member.isReference()) {
          if (!inheritedDescription && memberExpanded.description) inheritedDescription = memberExpanded.description;
        } else if (!description && memberExpanded.description) {
          description = memberExpanded.description;
        }
      }

      return { kind: 'object', properties, required, description: description || inheritedDescription };
    }
  ...
  _buildDescriptionCell(propSchema, desc, expanded) {
    if (propSchema.description) return mdToHtml(propSchema.description);

    if (desc.refName) {
      const link = `<a href="#def-${this._esc(desc.refName)}" class="schema-ref">${this._esc(desc.refName)}${desc.description ? ': ' : ' '}</a>`;
      return desc.description ? `${link}${mdToHtml(desc.description)}` : link;
    }

    if (desc.description) return mdToHtml(desc.description);
    if (expanded.description) return mdToHtml(expanded.description);
    return '';
  }

  /**
   * Descreve um schema já _expandido: rótulo de TIPO LIMPO pra célula "Tipo"
  ...
  _describe(expanded, visited) {
    if (expanded.kind === 'primitive') {
      return { cssType: expanded.label, type: expanded.label, description: expanded.description, refName: null, next: null };
    }
  ...
  _collectExampleRows(properties, required, depth, visited, rows) {
		let nProperties = properties.size
    for (const [name, propSchema] of properties) {
      const expanded = this._expand(propSchema, visited);
			const type = expanded.type || expanded.kind;
			const comma = (--nProperties > 0)

      if (expanded.kind === 'primitive') {
        rows.push({ depth, type, name, value: `<span class="${expanded.type}">${this._formatExampleValue(this._generateExampleValue(expanded.schema)) }</span>`, comma });
        continue;
      }

      if (expanded.kind === 'cycle') {
        rows.push({ depth, type, name, value: '<em>(circular)</em>', comma });
        continue;
      }

      if (expanded.kind === 'object') {
        rows.push({ depth, type, name, tag: '{' });
        const nextVisited = expanded.refName ? new Set(visited).add(expanded.refName) : visited;
        this._collectExampleRows(expanded.properties, expanded.required, depth + 1, nextVisited, rows);
				rows.push({ depth, tag: '}', comma });
        continue;
      }
      ...
      if (itemExpanded.kind === 'primitive') {
        ...
        rows.push({ depth, type, name, tag: '<span class="array">[</span>', value: `<span class="${itemExpanded.type}">${this._formatExampleValue(val)}</span>`, end: '<span class="array">[</span>', comma });
      } else if (itemExpanded.kind === 'object') {
        rows.push({ depth, type, name, tag: '<span class="array">[</span> {' });
        ...
        rows.push({ depth, tag: '} <span class="array">]</span>', comma });
      } else {
        rows.push({ depth, type, name, value: '<em>(circular)</em>', comma });
      }
    }
...
<div class="definition" id="def-${this._esc(name)}">
		<div class="header">
			...
			${schema.description ? `– <span class="description">${mdToHtml(schema.description)}</span>` : ''}
		</div>
  ${schema.accept(this)}
</div>`.trim();
  }
    ...
    if (str === null || str === undefined) return '';
		return `<span>"<span class="key">${this._esc(str)}</span>"<span class="colon">:</span> </span> `;
	}
