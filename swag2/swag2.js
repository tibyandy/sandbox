...
class Schema {
  constructor({
    ...
    this.example = example; // valor cru de json.example, se houver (pode ser qualquer tipo)
  ...
  static fromJSON(json, registry, name = null) {
    ...
    return new Schema({
      ...
      example: json.example, // undefined se ausente — diferenciamos de "example: null" explícito
      raw: json,
    });
  }
...
class Operation {
  constructor({ path, method, operationId, summary, description, tags, parameters, responses, consumes, produces }) {
    this.path = path;
    this.method = method;
  ...
  static fromJSON(path, method, json, registry, tagsRegistry) {
    ...
    const operation = new Operation({
      ...
      tagsRegistry.put(tag, operation)
    }
...
class TagOperationsRegistry {
  constructor() {
    this.tagOperations = [];
  }
	put(tag, operation) {
		this.tagOperations[tag] ||= new TagOperations(tag);
		this.tagOperations[tag].put(operation)
	}
	values() {
		return Object.values(this.tagOperations)
	}
}
...
class PathItem {
  ...
  static fromJSON(path, json, registry, tagsRegistry) {
    const operations = new Map();
    for (const method of HTTP_METHODS) {
      if (json[method]) {
        operations.set(method, Operation.fromJSON(path, method, json[method], registry, tagsRegistry));
      }
    }
...
class SwaggerDocument {
  constructor() {
    ...
    this.registry = new SchemaRegistry();
		this.tagsRegistry = new TagOperationsRegistry();
  ...
  static parse(json) {
    ...
    for (const [path, pathJson] of Object.entries(json.paths || {})) {
      doc.paths.set(path, PathItem.fromJSON(path, pathJson, doc.registry, doc.tagsRegistry));
    }
...
class SwaggerVisitor {
  ...
  visitTag(tag) { throw new Error('visitTag não implementado'); }
